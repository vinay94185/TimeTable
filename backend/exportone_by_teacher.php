...
//  $result = get_by_sql("SELECT * FROM `timetable` where teacher='".$selected."' ORDER BY period");
  $table = [];

  foreach($weekDays as $day) {
    $result = get_by_sql("SELECT * FROM `timetable` where teacher='$selected' AND day='$day' ORDER BY period");
    if(!empty($result)) {
